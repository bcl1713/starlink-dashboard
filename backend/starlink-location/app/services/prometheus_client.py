...
class MonitoringPrometheusClient:
    ...
    async def get_history(
        ...
        *,
        ...
        await self._enforce_rate_limit(client_id)
        end = self._utc_now()
        key = (
        ...
        async with self._flight_lock:
            ...
            if flight is None:
                ...
                task.add_done_callback(
                    lambda _task, flight_key=key: asyncio.create_task(
                        self._cleanup_flight(flight_key)
                    )
                )
            flight.waiters += 1
            return flight

    async def _cleanup_flight(self, key: tuple[int, int, int]) -> None:
        async with self._flight_lock:
            flight = self._flights.get(key)
    ...
    async def _fetch_history(
        ...
        tasks = [
            self._fetch_series(
                metric, expr, start, end, request.step_seconds, max_points
            )
    ...
    async def _fetch_series(
        ...
        samples = self._parse_prometheus_response(decoded, expr, max_points)
    ...
    def _parse_prometheus_response(
        ...
        if not isinstance(series, dict) or set(series) != {"metric", "values"}:
            raise MonitoringPrometheusError("bad_shape")
        if series["metric"] != {"__name__": expr} or not isinstance(
            series["values"], list
        ):
