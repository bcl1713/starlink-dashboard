...
class PositionData(BaseModel):
    ...
    altitude: float = Field(..., description="Altitude in feet above sea level")
    speed: float = Field(default=0.0, description="Speed in knots")
    heading: float = Field(
        default=0.0, description="Heading in degrees (0-360, 0=North)"
    )
...
class NetworkData(BaseModel):
    ...
    throughput_down_mbps: float = Field(..., description="Download throughput in Mbps")
    throughput_up_mbps: float = Field(..., description="Upload throughput in Mbps")
