...
class RouteConfig(BaseModel):
    ...
    pattern: Literal["circular", "straight"] = Field(
        default="circular", description="Route pattern: circular or straight line"
    )
    latitude_start: float = Field(
        default=40.7128, description="Starting latitude (degrees)"
    )
    longitude_start: float = Field(
        default=-74.0060, description="Starting longitude (degrees)"
    )
    radius_km: float = Field(
        default=100.0, description="Radius for circular route (kilometers)"
    )
    distance_km: float = Field(
        default=500.0, description="Total distance for straight route (kilometers)"
    )
...
class NetworkConfig(BaseModel):
    ...
    latency_min_ms: float = Field(
        default=20.0, description="Minimum latency (milliseconds)"
    )
    latency_typical_ms: float = Field(
        default=50.0, description="Typical latency (milliseconds)"
    )
    latency_max_ms: float = Field(
        default=80.0, description="Maximum latency without spikes (milliseconds)"
    )
    latency_spike_max_ms: float = Field(
        default=200.0, description="Maximum latency with spikes (milliseconds)"
    )
    spike_probability: float = Field(
        default=0.05, description="Probability of latency spike (0.0-1.0)"
    )

    throughput_down_min_mbps: float = Field(
        default=50.0, description="Minimum download throughput (Mbps)"
    )
    throughput_down_max_mbps: float = Field(
        default=200.0, description="Maximum download throughput (Mbps)"
    )
    throughput_up_min_mbps: float = Field(
        default=10.0, description="Minimum upload throughput (Mbps)"
    )
    throughput_up_max_mbps: float = Field(
        default=40.0, description="Maximum upload throughput (Mbps)"
    )

    packet_loss_min_percent: float = Field(
        default=0.0, description="Minimum packet loss percentage (0.0-100.0)"
    )
    packet_loss_max_percent: float = Field(
        default=5.0, description="Maximum packet loss percentage (0.0-100.0)"
    )
...
class ObstructionConfig(BaseModel):
    ...
    min_percent: float = Field(
        default=0.0, description="Minimum obstruction percentage"
    )
    max_percent: float = Field(
        default=30.0, description="Maximum obstruction percentage"
    )
    variation_rate: float = Field(
        default=0.5, description="Rate of obstruction variation (0.0-1.0)"
    )
...
class PositionConfig(BaseModel):
    ...
    speed_min_knots: float = Field(default=0.0, description="Minimum speed (knots)")
    speed_max_knots: float = Field(default=100.0, description="Maximum speed (knots)")
    altitude_min_feet: float = Field(
        default=328.0, description="Minimum altitude in feet (default ~100 meters)"
    )
    altitude_max_feet: float = Field(
        default=32808.0, description="Maximum altitude in feet (default ~10000 meters)"
    )
    heading_variation_rate: float = Field(
        default=5.0, description="Rate of heading variation (degrees per update)"
    )
...
class SimulationConfig(BaseModel):
    ...
    update_interval_seconds: float = Field(
        default=1.0, description="Interval between metric updates (seconds)"
    )
    route: RouteConfig = Field(
        default_factory=RouteConfig, description="Route simulation configuration"
    )
    ...
    position: PositionConfig = Field(
        default_factory=PositionConfig, description="Position simulation configuration"
    )
