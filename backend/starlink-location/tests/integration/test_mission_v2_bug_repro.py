...
def test_delete_leg_scope_bug(client: TestClient):
    ...
    from app.mission.models import MissionLegTimeline, TimelineSegment, TimelineStatus
    ...
    with patch("app.mission.routes_v2.build_mission_timeline", return_value=(timeline, summary)):
    ...
    assert len(remaining_pois) == 1, f"Expected 1 POI remaining, found {len(remaining_pois)}"
