...
The mission export system comprises two interdependent modules (3,518 total lines) responsible for transforming mission timeline data into deliverable formats and portable mission archives.
...
- `ExportArtifact` - Container for generated export (content, media_type, extension)
...
**Responsibility:** Create portable mission archives with all legs, routes, POIs, and exports
...
- Timestamp formatting, data processing, visualization rendering, and packaging all tightly coupled
...
- PDF pagination logic appears in both `generate_pptx_export()` and `generate_mission_combined_pptx()`
...
- Columns: Segment #, Mission ID, Status, Start/End times (UTC/Eastern/T+), Duration, Transport states, Impacted systems, Reasons, Metadata
- Generated from `_segment_rows()` DataFrame

### XLSX Export
- Multi-sheet workbook:
  1. **Summary**: Map image (750x500px), Timeline chart (850x300px), Summary table with color coding
  2. **Timeline**: Full segment rows (detailed version)
  3. **Advisories**: Timeline advisory events (if any)
  4. **Statistics**: Mission metrics (if any)
...
- Advanced styling: Row alternation, per-column coloring, safety-of-flight detection
...
### Configuration
- `EASTERN_TZ = ZoneInfo("America/New_York")` - Hardcoded, consider parametrization
- `LOGO_PATH = assets/logo.png` - Must exist for header image
- Sheet name limit: 31 characters (Excel standard, enforced in package_exporter.py)

---

## Future Enhancement Opportunities

1. **Streaming Exports:** Replace full buffering with streaming for large archives
2. **Format Plugins:** Plugin architecture for custom export formats
3. **Async Processing:** Async/celery for long-running exports
4. **Caching:** Cache DataFrames and images to reduce re-computation
5. **Configuration:** Externalize colors, formats, templates
6. **Export Templates:** Custom PDF/PowerPoint templates
7. **Database Exports:** Direct database export (CSV/Parquet)
8. **Real-time Updates:** WebSocket for export progress
9. **Batch Operations:** Export multiple missions in one request
10. **Archive Formats:** Tar.gz, tar.bz2, 7z support
