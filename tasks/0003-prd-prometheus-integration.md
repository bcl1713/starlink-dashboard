...
time-series data collection from both simulated and live Starlink terminal data,
providing the foundation for Grafana visualization and historical analysis.
...
1. Expose all Starlink telemetry metrics via a Prometheus-compatible `/metrics`
   endpoint
1. Configure Prometheus to scrape metrics at intervals suitable for real-time
   monitoring (10-15 seconds)
1. Implement appropriate metric types (Gauges, Histograms, Counters) based on
   data characteristics
1. Add rich metric labels for filtering by mode, status, and geographic context
1. Include comprehensive health checks and meta-metrics for monitoring system
   health
1. Ensure extensible data retention configuration via environment variables
1. Provide full test coverage (unit, integration, validation)
1. Document all exposed metrics with query examples
...
1. **As an operator**, I want to filter metrics by `mode="simulation"` or
   `mode="live"` so I can distinguish between test and production data.

1. **As a dashboard designer**, I want histogram metrics for latency and
   throughput so I can create percentile-based visualizations (p50, p95, p99).

1. **As a system administrator**, I want health check endpoints that expose
   Prometheus scraping status so I can monitor the monitoring system itself.

1. **As a user**, I want to configure Prometheus retention via the
   `PROMETHEUS_RETENTION` environment variable so I can balance storage costs
   with historical analysis needs.

1. **As a developer**, I want comprehensive metric documentation with example
...
1. `starlink_dish_latitude_degrees` - Current latitude (-90 to 90)
2. `starlink_dish_longitude_degrees` - Current longitude (-180 to 180)
3. `starlink_dish_altitude_meters` - Current altitude above sea level
4. `starlink_dish_speed_knots` - Current ground speed in knots
5. `starlink_dish_heading_degrees` - Current heading (0-360, true north)

### 4.3 Core Metrics - Network Performance (Histograms + Gauges)

1. `starlink_network_latency_ms` - Histogram with buckets [20, 40, 60, 80, 100,
   150, 200, 500]
1. `starlink_network_latency_ms_current` - Gauge for latest value
1. `starlink_network_throughput_down_mbps` - Histogram with buckets [50, 100,
   150, 200, 250, 300]
1. `starlink_network_throughput_down_mbps_current` - Gauge for latest value
1. `starlink_network_throughput_up_mbps` - Histogram with buckets [10, 20, 30,
   40, 50]
1. `starlink_network_throughput_up_mbps_current` - Gauge for latest value

### 4.4 Core Metrics - Status (Gauges)

1. `starlink_dish_obstruction_percent` - Obstruction percentage (0-100)
2. `starlink_dish_uptime_seconds` - Uptime since last boot
3. `starlink_dish_thermal_throttle` - Boolean (0 or 1) indicating thermal
   throttling
4. `starlink_dish_outage_active` - Boolean (0 or 1) indicating active outage

### 4.5 Core Metrics - POI/ETA (Gauges with labels)

1. `starlink_eta_poi_seconds{name="POI_NAME"}` - ETA to each POI in seconds
2. `starlink_distance_to_poi_meters{name="POI_NAME"}` - Distance to each POI in
   meters

### 4.6 Event Metrics (Counters)

1. `starlink_connection_attempts_total` - Total connection attempts to dish
2. `starlink_connection_failures_total` - Failed connection attempts
3. `starlink_outage_events_total` - Total outage events detected
4. `starlink_thermal_events_total` - Total thermal throttle events

### 4.7 Metric Labels

1. All metrics MUST include a `mode` label with values `simulation` or `live`.
2. Position metrics SHOULD include `region` and `zone` labels when geographic
   context is available.
3. Network metrics SHOULD include a `status` label indicating connection quality
   (`excellent`, `good`, `degraded`, `poor`).

### 4.8 Prometheus Configuration

1. Prometheus MUST scrape the backend at a 10-second interval (balancing
   real-time needs with load).
1. The scrape configuration MUST be defined in
   `monitoring/prometheus/prometheus.yml`.
1. The scrape timeout MUST be set to 5 seconds.
1. Prometheus MUST use the `PROMETHEUS_RETENTION` environment variable for data
   retention (default: `15d`).

### 4.9 Health Checks

1. The backend MUST expose a `/health` endpoint returning JSON:
...
1. The `/health` endpoint MUST return HTTP 200 when healthy, 503 when degraded.

### 4.10 Meta-Metrics

1. The backend MUST expose `starlink_metrics_scrape_duration_seconds` -
   Histogram of metric collection time.
1. The backend MUST expose `starlink_metrics_generation_errors_total` - Counter
   of errors during metric generation.
1. The backend MUST expose `starlink_metrics_last_update_timestamp_seconds` -
   Unix timestamp of last successful update.

### 4.11 Testing

1. Unit tests MUST verify all metric types are correctly instantiated.
2. Unit tests MUST verify metric values fall within expected ranges (e.g.,
   latitude -90 to 90).
3. Integration tests MUST verify Prometheus can successfully scrape the
   `/metrics` endpoint.
4. Integration tests MUST verify metrics appear in Prometheus with correct
   labels.
5. Validation tests MUST check that histogram buckets contain appropriate data
   distributions.
6. Tests MUST cover both simulation and live mode scenarios.

### 4.12 Documentation

1. A `METRICS.md` file MUST document all exposed metrics with descriptions,
   types, and labels.
1. The documentation MUST include example Prometheus queries for common use
   cases:
   ...
   - Outage frequency
   - ETA to specific POI
1. Inline code comments MUST explain metric bucket choices and label logic.
...
1. **Custom metric aggregation** - Prometheus recording rules will be added in a
   future phase.
1. **Alerting rules** - Alert configuration is deferred to Phase 6+.
1. **Push-based metrics** - This phase uses pull-based scraping only; push
   gateways are out of scope.
1. **Multi-terminal support** - Single dish monitoring only; multi-instance
   support is a future enhancement.
1. **Authentication** - Metrics endpoint will be unauthenticated (internal
   network only).
1. **Custom exporters** - No integration with `speedtest-exporter` or
...
- Failed metric updates MUST increment
...
1. **Metric Naming:** Confirm naming convention follows Prometheus best
   practices (base unit in metric name, e.g., `_seconds`, `_meters`).
   - **Decision:** Yes, follow standard conventions per Prometheus docs.

1. **Remote Write:** Should we support Prometheus remote write for external
   storage (e.g., Grafana Cloud)?
   - **Decision:** Out of scope for Phase 3, add as optional Phase 9
     enhancement.

1. **Histogram vs Summary:** Should network metrics use Summary instead of
...
1. **Cardinality Monitoring:** Should we expose cardinality metrics for POI
   labels?
